...
port = serial.Serial("/dev/serial0", baudrate=115200, bytesize=serial.EIGHTBITS, parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE, timeout=1)
...
write_id = bytearray(b'\xab\xba\x00\x16\x05\x00\x00\x03\x92\x4d\xcf')
...
def main():
    ...
    choice = int(input("Choose option from menu: "))
    payload = menu[choice]
    ...
    if (choice == 4):
       ...
       prefix = "abba001605" # prefix abba, address 00, instruction code 16, payload length 05
       new_id = input("Enter new ID to write: ")
       while len(new_id) != 10:
           new_id = input ("Enter new ID to write with correct length 10: ")
       write_payload = bytearray.fromhex(f"{prefix}{new_id}")
       checksum = 0
       for byte in write_payload[2:]:
           checksum ^= byte
       write_payload.append(checksum)
       print(write_payload)
    ...
    while True:
        ...
        if (choice == 7):
            ...
            hex_o = rcv.hex()
            print(f"Raw hex output: {hex_o}")
            print(f"Explanation - prefix:{hex_o[:4]}, address:{hex_o[4:6]}, response code: {hex_o[6:8]}, response length: {hex_o[8:10]}, checksum:{hex_o[-2:]}")
            print(f"Output: {hex_o[10:-2]}")
        time.sleep(2)
